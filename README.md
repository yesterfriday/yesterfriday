# 👋 Hi, I'm 재찬 (yesterfriday)
Unity Developer · ML/RL Researcher · Game Designer

---

## 🚀 About Me
- Unity 클라이언트와 Spring Boot 서버를 연동한 클라이언트-서버 게임 개발 경험
- Unity 기반 보드게임 개발(MediaProject_BoardGame)
- Photon Fusion 멀티플레이 구현 경험
- Unity UPM 패키지 구조로 재사용 가능한 게임플레이 시스템 개발 경험
- ML-Agents 환경 구축 및 DDPG/PPO 실험 진행
- Kaggle 및 머신러닝 실습 프로젝트 경험
- UI/UX · 게임기획 · Figma 작업 진행 중

---

## 🔥 Featured Projects

### 📦 BoxBox (Unity + Spring Boot)
2D 탑뷰 로컬 협동 물류 액션 게임 개발 프로젝트
- 팀 리더 및 서버 개발 담당
- Unity 클라이언트와 Spring Boot 서버 REST API 연동
- 로그인 / 점수 저장 / 스테이지별 리더보드 시스템 구현
- OpenWeather API를 활용한 도시 기반 날씨 시스템 구현
- 날씨에 따라 Fire / Puddle 장애물 생성
- FSM 기반 방해 NPC 시스템 적용
- 제한 시간, 목표 점수, 결과 화면을 포함한 게임 루프 구성  
👉 https://github.com/12e0hw/MSE_BoxBox

---

### 🎮 MediaProject BoardGame (Unity + Photon)
멀티플레이 보드게임 개발 프로젝트  
- 랜덤 맵 생성(MapGraphGenerator)  
- Dice System / Turn Manager / Player Movement  
- Fusion 네트워크 이동 동기화  
👉 https://github.com/yesterfriday/MediaProject_BoardGame

---

# 🚗 Unity 3D Driving Simulator with Logitech Steering Wheel (G29)
- Logitech G29 스티어링 휠을 Unity와 연동하여 3D 주행 시뮬레이터를 제작  
- 핸들 조작, 페달 입력, Force Feedback, 차량 물리 엔진, UI HUD 등을 구현  
👉 https://github.com/WhySongSerious/GyoAJung_TrafficQuest

---

### 🧩 Unity Gameplay Common Systems (Unity UPM Package)
재사용 가능한 Unity 게임플레이 공통 시스템 패키지 개발 프로젝트  
- Unity Package Manager(UPM) 구조로 모듈화  
- Inventory / Cards / Monster Spawner / Slingshot2D 시스템 구현  
- 각 시스템별 샘플 씬 및 검증용 Sample Hub 구성  
- ScriptableObject 기반 데이터 정의 및 게임 로직 분리  
- 1-scene MVP 샘플(GameA)을 통해 wave → targeting → cards → loot → inventory 흐름 테스트  
👉 https://github.com/yesterfriday/unity-gameplay-common-systems

---

### 📊 Kaggle – Bike Demand Forecasting
Bike sharing 데이터 기반 수요 예측  
- Feature Engineering  
- Ensemble 모델 비교

---

## 🛠 Tech Stack
- **Unity**, C#, Photon Fusion  
- **Python**, PyTorch, TensorFlow
- **Unity Package**: UPM, ScriptableObject, Sample Scene, Gameplay Module Design
- **Backend/Game Server**: Spring Boot, Java, H2, REST API, OpenWeather API
- **ML/RL**: PPO, DDPG, DQN, ML-Agents  
- Git, GitHub, Rider, Notion, Figma

---

## 📫 Contact
Email: qwe50922327@gmail.com
Notion Portfolio: (링크 추가 예정)
